export default class WaterTempertureGetter {
	...
	getWaterTemp(pin) {
		...
		return this.board.getWaterTemp(pin).then((data) => data / 10);
	}
	...
	async getWaterTemperatureA() {
		...
		if (
			Date.now() - this.waterTempAUpdatedTime >
			this.waterTempAUpdateIntervalTime
		) {
	...
	async getWaterTemperatureB() {
		...
		if (
			Date.now() - this.waterTempBUpdatedTime >
			this.waterTempBUpdateIntervalTime
		) {
