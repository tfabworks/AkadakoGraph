...
export default class DistanceGetter {
	...
	async getOpticalDistanceSensor() {
		if (!this.vl53l0x) {
			...
			if (
				this.board.version.type <= 1 ||
				(this.board.version.type === 2 &&
					this.board.version.major === 0 &&
					this.board.version.minor === 0)
			) {
				address = null;
			}
	...
	async measureDistanceWithLight() {
		...
		if (
			Date.now() - this.opticalDistanceUpdatedTime >
			this.opticalDistanceUpdateIntervalTime
		) {
	...
	async measureDistanceWithUltrasonicA() {
		...
		if (
			Date.now() - this.sonicDistanceAUpdatedTime >
			this.sonicDistanceAUpdateIntervalTime
		) {
			...
			const pin = 10;
			getter = getter
				.then(() => this.board.getDistanceByUltrasonic(pin))
				.then((value) => {
	...
	async measureDistanceWithUltrasonicB() {
		...
		if (
			Date.now() - this.sonicDistanceBUpdatedTime >
			this.sonicDistanceBUpdateIntervalTime
		) {
