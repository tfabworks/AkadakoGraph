...
export default class EnvSensorGetter {
	...
	async getEvnTemperature() {
		...
		if (
			Date.now() - this.envTemperatureUpdatedTime >
			this.envTemperatureUpdateIntervalTime
		) {
		...
		return getter
			...
			.catch((reason) => {
				console.error(
					`getting environment temperature was rejected by ${reason}`,
				);
				this.envTemperature = null;
				this.envSensor = null;
				return null;
	...
	async getEnvPressure() {
		...
		if (
			Date.now() - this.envPressureUpdatedTime >
			this.envPressureUpdateIntervalTime
		) {
	...
	async getEnvHumidity() {
		...
		if (
			Date.now() - this.envHumidityUpdatedTime >
			this.envHumidityUpdateIntervalTime
		) {
