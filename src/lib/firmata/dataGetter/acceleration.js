...
export default class AccelerationGetter {
	...
	async updateAcceleration() {
		...
		if (
			Date.now() - this.accelerationUpdatedTime >
			this.accelerationUpdateIntervalTime
		) {
	...
	getAccelerationAbsolute() {
		...
		return updateAcc.then((acc) => {
			if (!acc) return null;
			const absolute = Math.sqrt(acc.x ** 2 + acc.y ** 2 + acc.z ** 2);
	...
	getPitch() {
		...
		return updateAcc.then((acc) => {
			if (!acc) return null;
			const angle =
				(Math.atan2(acc.x, Math.sqrt(acc.y * acc.y + acc.z * acc.z)) * 180.0) /
				Math.PI;
			let pitch = angle;
			if (acc.z < 0) {
