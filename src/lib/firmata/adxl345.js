...
export default class ADXL345 {
	 ...
	 */
	init() {
		return this.readID().then((id) => {
			if (id !== ADXL345_ID)
				return Promise.reject(`0x${this.address.toString(16)} is not ADXL345`);
	...
	readID() {
		return this.board
			.i2cReadOnce(this.address, 0x00, 1, this.timeout)
			.then((data) => data[0]);
	}
	...
	getAcceleration() {
		return this.board
			.i2cReadOnce(this.address, DATA_X0, 6, this.timeout)
			.then((data) => {
