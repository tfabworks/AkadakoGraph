...
const timeoutReject = (delay) =>
	new Promise((_, reject) =>
		setTimeout(() => reject(`timeout ${delay}ms`), delay),
	);
...
class AkaDakoBoard extends EventEmitter {
	...
	async openSerialPort(options) {
		...
		if (
			permittedPorts !== null &&
			Array.isArray(permittedPorts) &&
			permittedPorts.length > 0
		) {
	...
	async connectSerial(options) {
		...
		return Promise.race([
			request,
			timeoutReject(this.connectingWaitingTime),
		]).catch((reason) => {
			this.releaseBoard();
		...
		if (filters) {
			for (const filter of filters) {
				...
				midiAccess.inputs.forEach((port) => {
					if (
						(!filter.manufacturer ||
							filter.manufacturer.test(port.manufacturer)) &&
	...
	onBoardReady() {
		console.log(
			`${this.version.type}.${String(this.version.major)}.${String(
				this.version.minor,
	...
	boardVersion(timeout) {
		if (this.version)
			return Promise.resolve(
				`${this.version.type}.${this.version.major}.${this.version.minor}`,
			);
