...
export default class BME280 {
	...
	read8(register) {
		return this.board
			.i2cReadOnce(this.address, register, 1, this.timeout)
			.then((data) => data[0]);
	}
	...
	readUint16BE(register) {
		return this.board
			.i2cReadOnce(this.address, register, 2, this.timeout)
			.then((data) => {
	...
	read24(register) {
		return this.board
			.i2cReadOnce(this.address, register, 3, this.timeout)
	...
	async init() {
		...
		this.dig_H4 =
			((await this.read8(BME280_REG_DIG_H4)) << 4) |
			(0x0f & (await this.read8(BME280_REG_DIG_H4 + 1)));
		this.dig_H5 =
			((await this.read8(BME280_REG_DIG_H5 + 1)) << 4) |
