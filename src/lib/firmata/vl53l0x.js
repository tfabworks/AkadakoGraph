...
export default class VL53L0X {
	...
	async init(io2v8) {
		...
		if (io2v8) {
			this.writeReg(
				...
				(await this.readReg(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV)) | 0x01,
			); // set bit 0
		}
		...
		this.writeReg(
			MSRC_CONFIG_CONTROL,
			(await this.readReg(MSRC_CONFIG_CONTROL)) | 0x12,
		);
		...
		const refSpadMap = await this.readMulti(
		...
		this.writeReg(
			...
			(await this.readReg(GPIO_HV_MUX_ACTIVE_HIGH)) & ~0x10,
		); // active low
	...
	writeReg16Bit(register, value) {
		const data = [(value >> 8) & 0xff, value & 0xff];
	...
	readReg(register) {
		return this.board
			.i2cReadOnce(this.address, register, 1, this.io_timeout)
			.then((data) => data[0]);
	}
	...
	readReg16Bit(register) {
		return this.board
			.i2cReadOnce(this.address, register, 2, this.io_timeout)
			.then((data) => {
	...
	readReg32Bit(register) {
		return this.board
			.i2cReadOnce(this.address, register, 4, this.io_timeout)
			.then((data) => {
				const value =
	...
	checkTimeoutExpired() {
		return (
			this.io_timeout > 0 &&
			Date.now() - this.timeout_start_ms > this.io_timeout
		);
	}
	...
	async getSignalRateLimit() {
		return (
			(await this.readReg16Bit(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT)) /
			(1 << 7)
		);
	}
	...
	async setVcselPulsePeriod(type, period_pclks) {
		...
		if (type === VcselPeriodPreRange) {
			...
			const new_pre_range_timeout_mclks = this.timeoutMicrosecondsToMclks(
			...
			const new_msrc_timeout_mclks = this.timeoutMicrosecondsToMclks(
			...
			let new_final_range_timeout_mclks = this.timeoutMicrosecondsToMclks(
	...
	async getVcselPulsePeriod(type) {
		if (type === VcselPeriodPreRange) {
			return decodeVcselPeriod(
				await this.readReg(PRE_RANGE_CONFIG_VCSEL_PERIOD),
			);
		} else if (type === VcselPeriodFinalRange) {
			return decodeVcselPeriod(
				await this.readReg(FINAL_RANGE_CONFIG_VCSEL_PERIOD),
			);
		}
		return 255;
	}
		...
		while (((await this.readReg(RESULT_INTERRUPT_STATUS)) & 0x07) === 0) {
			if (this.checkTimeoutExpired()) {
				...
				return Promise.reject(
					`timeout read RESULT_INTERRUPT_STATUS: ${this.io_timeout}ms`,
				);
			}
		}
	...
	async readRangeSingleMillimeters() {
		...
		while ((await this.readReg(SYSRANGE_START)) & 0x01) {
			if (this.checkTimeoutExpired()) {
				this.did_timeout = true;
				return Promise.reject(
					`timeout read SYSRANGE_START: ${this.io_timeout}ms`,
				);
			}
		}
	...
	async getSequenceStepTimeouts(enables, timeouts) {
		timeouts.pre_range_vcsel_period_pclks =
			await this.getVcselPulsePeriod(VcselPeriodPreRange);

		timeouts.msrc_dss_tcc_mclks =
			(await this.readReg(MSRC_CONFIG_TIMEOUT_MACROP)) + 1;
		...
		timeouts.pre_range_mclks = this.decodeTimeout(
			await this.readReg16Bit(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI),
		);
		...
		timeouts.final_range_vcsel_period_pclks = await this.getVcselPulsePeriod(
			VcselPeriodFinalRange,
		);

		timeouts.final_range_mclks = this.decodeTimeout(
			await this.readReg16Bit(FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI),
		);
