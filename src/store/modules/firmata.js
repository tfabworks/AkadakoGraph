...
const milliSecondsList = [
	1000, 3000, 5000, 10000, 30000, 60000, 180000, 300000, 600000, 1800000,
];
...
const tmpAxisInfo = {
	...
	sub: parseInt(
		migrateSensorKind20230714(localStorage.getItem("graphKindSub")),
	),
};
...
const mutations = {
	addValue(state, { isMain, newValue }) {
		...
		const sensor = isMain
			? SensorMap.get(state.axisInfo.main.kind)
			: SensorMap.get(state.axisInfo.sub.kind);
		const targetGraphValue = isMain ? state.graphValue : state.graphValueSub;
		if (
			typeof targetGraphValue.length !== "undefined" &&
			targetGraphValue.length
		) {
		...
		if (isMain) {
			...
			if (state.axisInfo.main.dataCountSinceStart === 1) {
				if (typeof sensor.targetValueForCorrectionOnStart !== "undefined") {
					state.axisInfo.main.correctionRate =
						sensor.targetValueForCorrectionOnStart / newValueY;
				} else {
			...
			if (state.axisInfo.sub.dataCountSinceStart === 1) {
				if (typeof sensor.targetValueForCorrectionOnStart !== "undefined") {
					state.axisInfo.sub.correctionRate =
						sensor.targetValueForCorrectionOnStart / newValueY;
				} else {
			...
			if (typeof sensor.targetValueForCorrectionOnStart !== "undefined") {
				console.log({
					...
					targetValueForCorrectionOnStart:
						sensor.targetValueForCorrectionOnStart,
				});
			}
			newValue.y = correctedValueY;
			...
			localStorage.setItem(
				"graphValueSub",
				JSON.stringify(state.graphValueSub),
			);
		}
	},
...
const actions = {
	...
	async setValueToAdd(ctx) {
		if (
			ctx.state.board &&
			ctx.state.board.isConnected() &&
			!ctx.state.dataGetter
		) {
			ctx.state.dataGetter = new DataGetter(ctx.state.board);
		}
		...
		if (
			ctx.state.axisInfo.main.shouldRender &&
			ctx.state.axisInfo.sub.shouldRender
		) {
			// 両方の軸で描画する場合
			...
			//main軸だけ描画する場合
			const data = await ctx.state.dataGetter.getData(
				ctx.state.axisInfo.main.kind,
			);
			if (data !== null) {
				ctx.commit("addValue", {
					isMain: true,
